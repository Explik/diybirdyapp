describe('Flashcard deck features', () => {
  ...
  it ('Exercises are unavailable when no flashcards are present', () => {
    createNewDeck();
  ...
  it ('Exercises are available when flashcards are present', () => {
    createNewDeck(); 

    cy.contains('Add flashcard').click(); 
    cy.get('.ng-untouched > .border').type('Question 1')
    cy.get('.ng-untouched > .border').type('Answer 1')
